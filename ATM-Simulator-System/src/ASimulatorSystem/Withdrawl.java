...
public class Withdrawl extends JFrame implements ActionListener{
            ...
            if(ae.getSource() == b1){
                if(amount.equals("")){
                    ...
                    if(Integer.parseInt(amount) > 100000) {
                        ...
                        int option = JOptionPane.showConfirmDialog(null, pf, "Withdrawal amount exceeds Rs. 100,000. Please enter your pin:", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
                        if(option == JOptionPane.OK_OPTION) {
                            String enteredPin = new String(pf.getPassword());
                            if(!validatePin(enteredPin)) {
                                JOptionPane.showMessageDialog(null, "Invalid Pin. Transaction Cancelled.");
                                return;
                            }
                        } else {
                            return;
    ...
    private boolean validatePin(String enteredPin) {
        ...
        try {
            Conn c1 = new Conn();
            ResultSet rs = c1.s.executeQuery("select * from login where pin = '" + enteredPin + "'");
            if(rs.next()) {
                isValid = true;
            }
        } catch(Exception e) {
